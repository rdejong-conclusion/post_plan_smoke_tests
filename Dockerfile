FROM ubuntu:xenial
RUN apt-get update 
RUN apt-get install -y firefox xvfb python-pip ruby ruby-dev wget firefox
RUN apt-get remove -y firefox 
RUN wget https://ftp.mozilla.org/pub/firefox/releases/45.3.0esr/linux-x86_64/en-US/firefox-45.3.0esr.tar.bz2 -O /root/firefox.tar.bz2
RUN (cd /root/;tar -jxvf firefox.tar.bz2)
