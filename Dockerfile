...
CMD (/etc/init.d/xvfb start; export DISPLAY=":10" PATH="$PATH:/root/firefox/";cd /root/selenium_wd_tests;target_host=$target_host target_user=$target_user target_pass=$target_pass rspec post_plan_smoketest_ruby_webdriver --format RspecJunitFormatter --out results.xml)
